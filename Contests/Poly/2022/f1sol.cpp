...
void solve(){
    ...
    vi next_drep(n);
    forn(i, n){
        // if (next_rep[i] == n) next_drep[i] = replicate.size() - 1;
        // while (pt < replicate.size() && (d[replicate[pt]] - d[i] == 0)){
        //     pt++;
        // }
        // if (pt == replicate.size()) next_drep[i] = replicate.size() - 1;
        // else next_drep[i] = pt;
        int L = next_rep[i], R = replicate.size() - 1;
        while (L < R){
            int mid = (L + R) / 2;
            if (d[replicate[mid]] - d[i] > 0) R = mid;
            else L = mid + 1;
        }
        next_drep[i] = L;
    }
    ...
    for (pii p: spawn){
        ...
        if (cd[i] == 0 && d[n-1] - d[i] == 0){
            // cout << "one\n";
            if (j == n) ans += mint(1);
            else ans += pow_mod[replicate.size() - j]; 
            continue;
        }
        ...
        if (!flag){ ans += r * pow_mod[L - next_rep[i]]; continue;}
