...
mint binom(int n, int k){
    if (k < 0) return 0;
    if (k > n) return 0;
    return factor_mod[n] / factor_mod[k] / factor_mod[n-k];
}
...
void solve(){
    ...
    int l, r;
    forn(i, n+1){
        if (cnt[a[i]] == 2){l = i; break;}
    }

    per(i, n, 0){
        if (cnt[a[i]] == 2){r = n - i; break;}
    }

    // cout << l << " " << r << "\n";

    rep(k, 1, n+1){
        mint ans = binom(n-1, k) + 2 * binom(n-1, k-1) + binom(n-1, k-2) - binom(l+r, k-1);
        cout << ans << "\n";
    }
}
