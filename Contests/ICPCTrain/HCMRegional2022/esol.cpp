...
BigInt::BigInt(string & s){
    digits = {};
    int n = s.size();
    for (int i = n - 1; i >= 0;i--) digits.push_back(s[i] - '0');
}
...
BigInt &operator+=(BigInt &a,const BigInt& b){
    ...
    int n = Length(a), m = Length(b);
    if(m > n){
        forn(i, m-n) a.digits.push_back(0);
    }
    n = Length(a);
    for (i = 0; i < n;i++){
...
void solve(){
    ...
    forn(i, n-1){
        BigInt y1 = y[i], y2 = y[i+1], x1 = x[i], x2 = x[i+1];
        BigInt a, b, c;
        a = y1 - y2;
        b = x2 - x1;
        c = (y2 - y1) * x1 + (x1 - x2) * y1;
